    ...
    def move(self, player, column):
        ...
        for i in range(0, len(self.board)):
            if self.board[i][column] == 0:
                self.board[i][column] = self.player1 if player == 1 else self.player2
        ...
        for i in range(len(self.board)):
            for j in range(len(self.board[i])):
                ...
                if all(i + k < len(self.board) 
                       ...
                       for k in range(4)) \
                       or \
                    all(i + k < len(self.board) 
                       and j - k >= 0
                       and self.board[i + k][j - k] == self.board[i][j]
                       for k in range(4)) \
                       or \
                    all(j + k < len(self.board[i]) 
                    ...
                    all(i + k < len(self.board) 
                       ...
                       for k in range(4)):
                    self.game_result = 'p1' if self.board[i][j] == self.player1 else 'p2'
                    return
